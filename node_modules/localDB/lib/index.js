...
db.existsSync = function(table){
	if( !cfg.isExistPath ) return false;
	if( fs.existsSync( path.join(cfg.path+"/"+table+".clary") ) ){
		return true;
	} else {
		return false;
	}
...
db.insert = function(insertData, fn){
	...
	fs.readFile(cfg.tablePath, "utf-8", function(err, data){
		...
		data.curindex++;
		insertData = jsonMerge({_id: md5(), id: data.curindex}, insertData);
	...
	fs.readFile(cfg.tablePath, "utf-8", function(err, data){
		...
		if( updateData instanceof Array ){
			for( var i=0; i<updateData.length; i++ ){
				for( var j=0; j<data.data.length; j++ ){
					if( updateData[i].id && updateData[i].id === data.data[j].id ){
			...
			for( var j=0; j<data.data.length; j++ ){
				if( updateData.id && updateData.id === data.data[j].id ){
